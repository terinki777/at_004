...
WHERE
  AIRCRAFTS_DATA.AIRCRAFT_CODE = '320' OR AIRCRAFTS_DATA.AIRCRAFT_CODE = '321' OR AIRCRAFTS_DATA.AIRCRAFT_CODE = '733';

--2.         Вывести список самолетов с кодом не на 3;
SELECT *
FROM LANIT.AIRCRAFTS_DATA
WHERE NOT AIRCRAFTS_DATA.AIRCRAFT_CODE LIKE '3%';

--3.         Найти билеты оформленные на имя «OLGA», с емайлом «OLGA» или без емайла;
SELECT
  LANIT.TICKETS.EMAIL,
  LANIT.TICKETS.PASSENGER_NAME
FROM LANIT.TICKETS
WHERE LANIT.TICKETS.PASSENGER_NAME LIKE '%OLGA%' OR
      (LANIT.TICKETS.PASSENGER_NAME LIKE '%OLGA%' AND LANIT.TICKETS.EMAIL LIKE '%OLGA%');

--4.         Найти самолеты с дальностью полета 5600, 5700. Отсортировать список по убыванию дальности полета;
SELECT *
FROM LANIT.AIRCRAFTS_DATA
WHERE RANGE = '5600' OR RANGE = '5700'
ORDER BY RANGE DESC;
...
FROM LANIT.FLIGHTS
WHERE
  (DEPARTURE_AIRPORT = 'SVO' OR DEPARTURE_AIRPORT = 'VKO' OR DEPARTURE_AIRPORT = 'DME')
  AND
